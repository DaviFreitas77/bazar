  ...
  return (
    <div className="group/native-select relative w-fit has-[select:disabled]:opacity-50" data-slot="native-select-wrapper">
      <select
        ...
        className={cn(
          "border-input placeholder:text-muted-foreground selection:bg-primary selection:text-primary-foreground dark:bg-input/30 dark:hover:bg-input/50 h-11 w-full min-w-0 appearance-none rounded-md border border-gray-300 bg-transparent px-3 py-2 pr-9 text-sm  transition-[color,box-shadow] outline-none disabled:pointer-events-none disabled:cursor-not-allowed",
...
function DropDown({ title, value, onChange, children,style }: DropDownProps) {
  return (
    <NativeSelect 
      className={`${style}`}
      value={value} 
    ...
    >
      <NativeSelectOption value="">{title}</NativeSelectOption>
      {children}
    </NativeSelect>
  );
}

export { NativeSelect, NativeSelectOptGroup, NativeSelectOption, DropDown };
