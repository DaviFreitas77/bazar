...
export function ShowProductsSearched() {
  ...
  const { data: productsSearched} = hookSearchParams(nameProduct);

  const { data: products, isLoading: loadingProduct } = useAllProducts();
  const limitedProducts = productsSearched?.slice(0, 6) ?? products?.slice(0, 6) ?? [];
  ...
  if (nameProduct && limitedProducts.length == 0) {
    return (
      <div className="col-span-2 flex flex-col justify-center items-center min-h-[80vh] text-gray-500">
        ...
        <p className="text-center text-lg font-medium mb-2">Nenhum produto encontrado</p>
