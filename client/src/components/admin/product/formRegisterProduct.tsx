...
import { apiCreateCategory } from "@/api/admin/categoryAdmin";
import { apiCreateSubCategory } from "@/api/admin/subCategorieApiAdmin";
...
export function FormRegisterProduct() {
    ...
    const [registerCategory, setRegisterCategory] = useState<boolean>(false)
    const [newModel, setNewModel] = useState<string>("")
    const [registerModel, setRegisterModel] = useState<boolean>(false)

    const [loading, setLoading] = useState<boolean>(false);
    const [uploading, setUploading] = useState<boolean>(false);
    const { data: categories, refetch } = useCategoriesAdmin();
    const { data: subCategories, refetch: refetchSub } = useListSubCategoriesById(Number(selectedCategory));
    ...
    const handleCreatedCategory = async () => {
        try {
            ...
            setRegisterCategory(false)
            setNewCategory("")
            refetch()
        } catch (error: any) {

            if (error.response.status == 409) {
                toast.error("Error essa categoria ja existe")
            }
            else {
                console.error(error)
    ...
    const handleCreatedModel = async () => {
        try {
            await apiCreateSubCategory(Number(selectedCategory), newModel)
                    ...
                    <div className="flex gap-6">
                        <div className="flex flex-col gap-1.5">
                            <div className="flex items-center gap-1">
                                ...
                                <button onClick={() => setRegisterCategory(!registerCategory)} type="button">
                                    <Plus size={15} className="hover:opacity-20 cursor-pointer" />
                                </button>
                            ...
                            {registerCategory && (
                                <div className="relative">
                                    <input
                                        ...
                                        type="text" placeholder="Nova categoria" className="border border-gray-200 px-2 text-sm rounded-md py-2 outline-none focus:border-primary-50 transition-all ease-in duration-150" />

                                    <button type="button" onClick={handleCreatedCategory}>
                                        <ArrowRight className="absolute top-3 right-2  cursor-pointer hover:text-primary-50" size={15} />
                                    </button>
                                </div>
                            )}
                        ...
                        <div className={`flex flex-col gap-1.5 ${subCategories ? "" : "hidden"}`}>
                            <div className="flex items-center gap-1">
                                ...
                                <button onClick={() => setRegisterModel(!registerModel)} type="button">
                                    <Plus size={15} className="hover:opacity-20 cursor-pointer" />
                                </button>
                            </div>
                            ...
                            {registerModel && (
                                <div className="relative">
                                    <input
                                        ...
                                        type="text" placeholder="Novo modelo" className="border border-gray-200 px-2 text-sm rounded-md py-2 outline-none focus:border-primary-50 transition-all ease-in duration-150" />

                                    <button type="button" onClick={handleCreatedModel}>
                                        <ArrowRight className="absolute top-3 right-2  cursor-pointer hover:text-primary-50" size={15} />
                                    </button>
                                </div>
                            )}
